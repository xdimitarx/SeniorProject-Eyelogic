...
bool Eye::findEyeCornerTMatching()
{
	...
	if(!templateLoaded)
	{
		if (leftEye)
		{
			cornerTemplate = Mat(filteredImage, Rect(filteredImage.cols*0.05, filteredImage.rows*0.2, filteredImage.cols*0.2, filteredImage.rows*0.6));
		}
		else
		{
			cornerTemplate = Mat(filteredImage, Rect(filteredImage.cols*0.65, filteredImage.rows*0.25, filteredImage.cols*0.3, filteredImage.rows*0.5));
		}
		templateLoaded = true;
	}
	...
	Rect templateLoc = Rect(matchLoc.x, matchLoc.y, 2, 2);
