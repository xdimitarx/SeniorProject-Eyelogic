...
Mat Sobel(Mat eyebox) {
	...
	for ( int y = 1; y < eyebox.rows- 2; x++) {
		for (int x = 1; x < eyebox.cols - 2; y ++) {
			firstPointSet = false;
			...
			//find line through point with gradient [pixel_y , pixel_x ]
			// 
			
			m = (point_y) / (point_x);
			yalpha = m*(0 - x) + y;
			ybeta  = m*(eyebox.cols - x) + y;
			xalpha = ((0-y) / m) + x;
			xbeta  = ((eyebox.rows - y) / m) + x;
